...
import { Stack } from '../../src/clients/portainer.js'
...
const { PortainerClient } = await import('../../src/clients/portainer.js')
...
describe('portainer-client', () => {
  ...
  const mockClient = new PortainerClient(
    'http://foo.test',
    'test-token',
    '1',
    'test-swarm-id'
  ...
  it('test deploy missing stack', async () => {
    ...
    expect(mockFetch).toHaveBeenLastCalledWith(
      'http://foo.test/api/stacks/create/swarm/string?endpointId=1',
      expect.anything()
    )
  ...
  it('test deploy existing stack', async () => {
    ...
    // make sure the api path correct
    expect(mockFetch).toHaveBeenLastCalledWith(
      'http://foo.test/api/stacks/1?endpointId=1',
      expect.anything()
    )
  ...
  it('test exception deploy', async () => {
    jest
      .spyOn(PortainerClient.prototype, 'isPresent')
